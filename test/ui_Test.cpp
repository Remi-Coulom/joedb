...
namespace joedb {
TEST(ui, client) {
  const std::array<const char *, 5> args{"--check", "none", "--db",
                                         "interpreted", "memory"};

  Arguments arguments(args.size(), args.data());

  Client_Parser parser(Open_Mode::shared_write, Client_Parser::DB_Type::none,
                       arguments);
  ...
  Writable_Client *writable_client =
      dynamic_cast<Writable_Client *>(parser.get());
...
TEST(ui, file_client) {
  const std::array<const char *, 8> args{
      "--check",    "none", "--db",        "interpreted",
      "test.joedb", "file", "interpreted", "connection_file.joedbi"};
  ...
  ASSERT_EQ(file_connection_string.str(), "create_table person\n"
